  ...
  <body>
    ...
    <section id="about" class="section">
        <div class="container">
            <div class="section-header">          
                ...
                <span>{{ $history->nama }}</span><br><br>
                @if ($history->foto)
                    <img src="{{ asset('gambar/'. $history->foto) }}" alt="Foto" width="400">
                @else
            ...
            <div class="container marketing">
                <div class="row featurette">
                    <div class="col-md-12">
                        <!-- deskripsi -->
                        <b class="size-text mb-2">Tanggal</b>
                        <p class="justify-content-between teks">{{ \Carbon\Carbon::parse($history->tanggal)->format('d-m-Y') }}</p>
                        <br>
                        <!-- deskripsi -->
                        <b class="size-text mb-2">Deskripsi</b>
                        <p class="justify-content-between teks">{{ $history->deskripsi }}</p>
                        <!-- anggaran proyek 
                        <div class="mt-4 mb-2"><b class="size-text">Data Anggaran</b></div>
                          <div class="table-responsive mb-10">
                              <table class="table teks">
                                  <thead>
                                      <tr>
                                          <th>Nama</th>
                                          <th>Anggaran</th>
                                  ...
                                  <tbody>
                                      {{-- @foreach($nama_anggaranArray as $key => $nama_anggaran)
                                          <tr>
                                              <td>{{ $nama_anggaran }}</td>
                                              <td>{{ isset($anggaranArray[$key]) ? $anggaranArray[$key] : '' }}</td>
                                          </tr>
                                      @endforeach --}}
                                  </tbody>
                              </table>
                        ...
                        <br>
                        -->
                        <!-- Kolom untuk menampilkan komentar -->
                        <br>
                        <b class="size-text">Komentar</b><br><br>
                        <div id="commentSection" class="comment-section">
                        @foreach ($comments as $comment)
                            <div id="comment_{{ $comment->id }}" class="comment">
                                <span class="comment-date">
                                    @if ($comment->user->level === 'admin')
                                        <span style="color: #EA752C">{{ $comment->user->name }}</span>
                                    @else
                                        {{ $comment->user->name }}
                                    @endif
                                    • {{ $comment->created_at->format('H:i') }} | {{ $comment->created_at->format('d/m/Y') }}
                                </span>
                                <p id="commentText_{{ $comment->id }}">{{ $comment->isi_komentar }}</p>
                                <div class="comment-actions button-container">
                                    @auth
                                        @if(Auth::user()->id === $comment->id_user || Auth::user()->level === 'admin')
                                            <button class="btn-edit" data-comment-id="{{ $comment->id }}">Edit</button>
                                            <b style="color:#EA752C"> | </b>
                                            <button class="btn-delete" data-comment-id="{{ $comment->id }}">Hapus</button>
                                        @endif
                                    @endauth
                                </div>
                            </div>
                        ...
                        <!-- Kolom untuk menginputkan komentar -->
                        <div class="mt-4">
                            @guest
                                <a href="{{ route('login') }}" class="btn-for-comment font-weight-bold"><i class="fas fa-arrow-right"></i> Login Terlebih Dahulu</a>
                            @else
                                <form id="commentForm" action="{{ route('komentar.store') }}" method="POST">
                                    @csrf
                                    <input type="hidden" id="id_histories" name="id_histories" value="{{ $history->id }}">
                                    <input type="hidden" id="id_user" name="id_user" value="{{ Auth::user()->id }}">
                                    <div class="form-group">
                                        <textarea class="form-control" id="isi_komentar" name="isi_komentar" rows="3" placeholder="Tulis komentar Anda di sini" required></textarea>
                                    </div>
                                    <button type="submit" class="btn btn-common">Tambah</button>
                                </form>
                            @endguest
                        </div>
                    </div>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', function() {
          // Menangani aksi edit komentar
          document.querySelectorAll('.btn-edit').forEach(button => {
              button.addEventListener('click', function() {
                  let commentId = this.getAttribute('data-comment-id');
                  let commentText = document.querySelector(`#commentText_${commentId}`).innerText;
                  document.querySelector('#editCommentTextarea').value = commentText;
                  document.querySelector('#editCommentForm').action = `/komentar/${commentId}`;
                  let editCommentModal = new bootstrap.Modal(document.getElementById('editCommentModal'));
                  editCommentModal.show();
              });
          });

          // Menangani aksi hapus komentar
